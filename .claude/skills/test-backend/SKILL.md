...
2. Read the route file being tested (derive from the test file name)
3. Read the model file if relevant
4. Read the reference test: `backend/tests/client.test.js`
5. Analyze the failure message — determine if the issue is in auth, endpoint path, assertion, or test ordering
6. Fix the test file using the Edit tool
7. If `--no-run` is present, **STOP** — do not run tests. Otherwise proceed to Step 4 (Run and Fix)
