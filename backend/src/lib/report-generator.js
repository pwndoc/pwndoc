...
const VulnerabilityCategory = require("../models/vulnerability-category");

// Generate document with docxtemplater
async function generateDoc(audit) {
  var templatePath = `${__basedir}/../report-templates/${audit.template.name}.${
    audit.template.ext || "docx"
  }`;
...
async function prepAuditData(data) {
  ...
  result.categories = await getCategoriesFromFindings(result.findings);
...
async function getCategoriesFromFindings(findings) {
  let categories = findings.reduce((categories, finding) => {
  ...
  for (const [key, category] of Object.entries(categories)) {
    const vulnerabilityCategory = await VulnerabilityCategory.getByName(
      category.name
    );

    categories[key] = {
      ...categories[key],
      description: await splitHTMLParagraphs(
        !!vulnerabilityCategory ? vulnerabilityCategory.description || "" : ""
      ),
    };
  }
