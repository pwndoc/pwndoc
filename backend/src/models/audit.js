  ...
  return new Promise((resolve, reject) => {
    ...
    return query
      ...
      .then((row) => {
        ...
        } else throw { fn: "NotFound", message: "Template not found" };
      })
      .then((rows) => {
        resolve(rows);
      })
...
AuditSchema.statics.getGeneral = (isAdmin, auditId, userId) => {
  return new Promise((resolve, reject) => {
    ...
    query.populate("company");
    query.select(
      "id name auditType location date date_start date_end client collaborators language scope.name template customFields"
    );
    query
      .exec()
      .then((row) => {
  ...
  return new Promise((resolve, reject) => {
    Audit.getLastFindingIdentifier(auditId).then((identifier) => {
      ...
      var query = Audit.findByIdAndUpdate(auditId, {
        $push: { findings: { $each: [finding], $sort: { cvssScore: -1 } } },
...
AuditSchema.statics.getLastFindingIdentifier = (auditId) => {
  return new Promise((resolve, reject) => {
    var query = Audit.aggregate([
      { $match: { _id: mongoose.Types.ObjectId(auditId) } },
    ]);
...
AuditSchema.statics.getFindings = (isAdmin, auditId, userId) => {
  return new Promise((resolve, reject) => {
    ...
    if (!isAdmin) query.or([{ creator: userId }, { collaborators: userId }]);
    query.select(
      "-_id findings._id findings.title findings.cvssSeverity findings.cvssScore"
    );
...
AuditSchema.statics.updateFinding = (
  ...
  return new Promise((resolve, reject) => {
    ...
    query
      ...
      .then(() => {
        return Audit.findByIdAndUpdate(auditId, {
          $push: {
            findings: { $each: [], $sort: { cvssScore: -1, priority: -1 } },
          },
        }).collation({ locale: "en_US", numericOrdering: true });
      })
      .then(() => {
