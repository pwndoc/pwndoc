  ...
  describe('Company Suite Tests', () => {
    ...
    describe('Company CRUD operations', () => {
      ...
      it('Update company with logo only', async done => {
        ...
        response = await request.put(`/api/companies/Company 1`, company, options)
        expect(response.status).toBe(200)

        response = await request.get('/api/companies', options)
        expect(response.data.datas[0].logo).toBe(logo)
        done()
      })

      it('Update company with nonexistent name', async done => {
        var company = {name: "company Updated"}

        response = await request.put(`/api/companies/Company nonexistent`, company, options)
        expect(response.status).toBe(404)
        done()
      })

      it('Delete company', async done => {
        var response = await request.delete(`/api/companies/Company 2`, options)
        expect(response.status).toBe(200)
      ...
      it('Delete company with nonexistent name', async done => {
        var response = await request.delete(`/api/companies/Company nonexistent`, options)
        expect(response.status).toBe(404)
        done()
      })
    })
