...
module.exports = function(request, app) {
    describe('Application configs', () => {
      ...
      it('Get configs', async done => {
          ...
          var response = await request(app).get('/api/configs', options);
      ...
      it('Edit configs', async done => {
        ...
        var response = await request(app).put('/api/configs', modified, options);
        expect(response.status).toBe(200);
        ...
        var response = await request(app).put('/api/configs', original, options);
        expect(response.status).toBe(200);
