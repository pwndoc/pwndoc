      ...
      >
        <template v-slot:before>
          <q-list class="home-drawer">
            ...
            <q-item :to="'/audits/' + auditId + '/general'" class="q-py-lg">
            ...
            <q-item :to="'/audits/' + auditId + '/network'" class="q-py-lg">
              <q-item-section avatar>
            ...
            <q-item class="q-py-lg">
              ...
              <q-item-section
                >Findings ({{ audit.findings.length || 0 }})</q-item-section
              >
              <q-item-section avatar>
                <q-btn
                  @click="
                    $router
                      .push('/audits/' + auditId + '/findings/add')
                      .catch((err) => {})
                  "
                  icon="add"
                  round
            ...
            <div
              v-for="categoryFindings of findingList"
              :key="categoryFindings.category"
            >
              <q-item-label header>{{
              ...
              <q-list
                no-border
                v-for="finding of categoryFindings.findings"
                :key="finding._id"
              >
                <q-item
                  dense
                    ...
                    <q-chip
                      ...
                      >{{
                        finding.cvssSeverity
                          ? finding.cvssSeverity.substring(0, 1)
                          : "N"
                      }}</q-chip
                    >
                  </q-item-section>
                ...
                <div class="row">
                  <div
                    v-for="(user, idx) in findingUsers"
                    :key="idx"
                    v-if="user.finding === finding._id"
                    class="col multi-colors-bar"
                    :style="{ background: user.color }"
                  />
                </div>
            ...
            <q-item class="q-py-lg">
              ...
              <q-item-section avatar>
                <q-btn round dense icon="add" color="secondary">
                  <q-menu
                  ...
                  >
                    <q-item v-close-popup>
                      <q-item-section
                        >No custom sections defined yet</q-item-section
                      >
                    </q-item>
                  </q-menu>
            ...
            <q-list v-for="section of audit.sections" :key="section._id">
              ...
              <div class="row">
                <div
                  v-for="(user, idx) in sectionUsers"
                  :key="idx"
                  v-if="user.section === section._id"
                  class="col multi-colors-bar"
                  :style="{ background: user.color }"
                />
              </div>
...
export default {
  ...
  computed: {
    generalUsers: function() {
      return this.users.filter((user) => user.menu === "general");
    },
    networkUsers: function() {
      return this.users.filter((user) => user.menu === "network");
    },
    findingUsers: function() {
      return this.users.filter((user) => user.menu === "editFinding");
    },
    sectionUsers: function() {
      return this.users.filter((user) => user.menu === "editSection");
    },
    ...
    findingList: function() {
      ...
      return _.chain(this.audit.findings)
        ...
        .map((value, key) => {
          ...
          else return { category: key, findings: value };
        })
        .value();
    },
  },
  ...
  methods: {
    ...
    getAudit: function() {
      AuditService.getAudit(this.auditId)
        .then((data) => {
          ...
          this.getSections();
          if (this.loading) this.handleSocket();
          this.loading = false;
        })
        .catch((err) => {
          if (err.response.status === 403)
            this.$router.push({
              name: "403",
              params: { error: err.response.data.datas },
            });
    ...
    getSectionIcon: function(section) {
      ...
      if (section) return section.icon || "notes";
      return "notes";
    },

    createSection: function(section) {
      AuditService.createSection(this.auditId, section).catch((err) => {
        Notify.create({
      ...
      AuditService.generateAuditReport(this.auditId)
        ...
        .catch(async (err) => {
          ...
          if (err.response && err.response.data) {
            var blob = new Blob([err.response.data], {
