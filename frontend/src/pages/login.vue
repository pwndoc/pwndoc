...
export default {
  ...
  created: function () {
    if (this.$route.query.tokenError)
      if (this.$route.query.tokenError === "2")
        this.errors.alert = "Expired token";
  ...
  methods: {
    ...
    checkInit() {
      Loading.show({
        message: "<p>Trying to contact backend</p>",
        customClass: "loading",
      ...
      UserService.isInit()
        ...
        .catch((err) => {
          Loading.show({
            message:
              "<i class='material-icons'>wifi_off</i><br /><p>Something went wrong contacting backend</p>",
            spinner: null,
...
<style lang="stylus">
.login-background {
    background: linear-gradient(45deg, #0D0D0D, #8C0303)
}
